...
from ..utils.general_utils import (
    PAGE,
    get_full_boilerplate_path,
    get_points_from_submission_info,
    get_problem_dir,
)
...
def add_submission(user: UserDep, session: SessionDep, submission: SubmissionRequest):
    ...
    if language is None or language.judge0id is None or language.id is None:
        raise HTTPException(
            ...
            detail="The language id provided does not belong to any language",
        )
    ...
    if problem is None or problem.id is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="The problem id provided does not belong to any problem",
        )

    if submission.source_code.strip() == "":
        raise HTTPException(
            status_code=status.HTTP_406_NOT_ACCEPTABLE,
            detail="Submission source code should contain more characters",
        )

    if submission.active_contest_id is not None:
        contest = session.exec(
            select(Contest).where(Contest.id == submission.active_contest_id)
        ).first()
        if contest is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Contest is not found"
            )
        now = datetime.now(tz=timezone.utc)
        is_contest_valid = contest.startTime <= now <= contest.endTime
        if not is_contest_valid:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Contest is not active"
            )
        ...
        if contest_problem_link is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="The problem is not part of this contest",
            )
    ...
    if total_test_cases <= 0 or len(only_test_cases) % total_test_cases != 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid test case file format"
        )

    num_cases_per_submission = len(only_test_cases) // total_test_cases

    submission_add_in_db = Submission(
        source_code=submission.source_code,
        problem_id=problem.id,
        language_id=language.id,
        user_id=user_id,
        status=SubmissionStatusId.IN_QUEUE,
        total_testcases=total_test_cases,
        total_passed_cases=0,
        active_contest_id=submission.active_contest_id,
    )
    ...
    if submission_add_in_db is None or submission_add_in_db.id is None:
        raise HTTPException(
            ...
            detail="Submission is invalid because of internal server error",
        )
    ...
    submissions = [
        Judge0RequestObject(
            ...
            stdin="\n".join(
                only_test_cases[
                    idx * num_cases_per_submission : (idx + 1) * num_cases_per_submission
                ]
            ),
        ).model_dump()
    ...
    with httpx.Client() as client:
        res = client.post(
            "http://server:2358/submissions/batch?base64_encoded=false",
            json={"submissions": submissions},
        )
    raw = res.json()
    response = [Judge0SubmitResponseObject.model_validate(item) for item in raw]
    test_cases_add_in_db = [
        TestCase(
            token=obj.token,
            status=SubmissionStatusId.IN_QUEUE,
            submission_id=submission_add_in_db.id,
            expected_output=expected_output[idx],
            stdin="\n".join(
                only_test_cases[
                    idx * num_cases_per_submission : (idx + 1) * num_cases_per_submission
    ...
    return SubmissionResponse(
        ...
        submission_id=submission_add_in_db.id,
        total_test_cases=total_test_cases if submission.active_contest_id is None else 0,
    )
...
def submission_webhook(body: SubmissionAPI, session: SessionDep):
    ...
    testcase_to_change.sqlmodel_update(body.model_dump(exclude_unset=True))
    testcase_to_change.status = SubmissionStatusId(
        body.status.id if body.status.id is not None else 1
    )
    ...
    submission = session.exec(
        select(Submission).where(Submission.id == testcase_to_change.submission_id)
    ).first()
    if submission is None or submission.id is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    error_free_statuses = [
        SubmissionStatusId.AC,
        SubmissionStatusId.IN_QUEUE,
        SubmissionStatusId.PROCESSING,
    ]
    ...
    if testcase_to_change.status == SubmissionStatusId.WA:
        ...
        if (
            submission.active_contest_id is not None
            and submission.contests_submissions_link is not None
        ):
            contest_submission = ContestSubmission(
                submission_id=submission.id,
                contest_id=submission.active_contest_id,
        ...
        if submission.total_passed_cases == submission.total_testcases:
            ...
            if (
                ...
                points_awarded = get_points_from_submission_info(
                    submission.active_contest.startTime,
                    submission.active_contest.endTime,
...
def get_submission_status(
    user: UserDep, session: SessionDep, submission_id: Annotated[int, Body(embed=True)]
):
    submission_test_cases = session.exec(
        select(TestCase).where(TestCase.submission_id == submission_id)
    ).all()
    submission = session.exec(
        select(Submission)
    ...
    if submission_test_cases is None or len(submission_test_cases) <= 0 or submission is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="The submission cannot be found"
        )
    if any(
    ...
    if is_wa:
        if submission.active_contest_id is not None:
            return SubmissionStatusResponse(
                ...
                message="Only truncated info will be shown for active contest",
                is_truncated_for_contest=True,
            )
        return SubmissionStatusResponse(
            state="FINISH",
            status=SubmissionStatusId.WA,
            max_memory=is_wa.memory,
            compile_output=is_wa.compile_output,
            total_time=str(float(is_wa.time or "0")),
            stderr=is_wa.stderr,
            stdout=is_wa.stdout,
            total_passed_cases=passed_count,
            total_testcases=len(submission_test_cases),
        )
    if is_ce:
        if submission.active_contest_id is not None:
            return SubmissionStatusResponse(
        ...
        return SubmissionStatusResponse(
            state="FINISH",
            status=SubmissionStatusId.CE,
            compile_output=is_ce.compile_output,
            total_passed_cases=passed_count,
            total_testcases=len(submission_test_cases),
        )
    if is_other_error:
        ...
        return SubmissionStatusResponse(
            state="FINISH",
            status=is_other_error.status,
            stderr=is_other_error.stderr,
            stdout=is_other_error.stdout,
            total_passed_cases=passed_count,
            total_testcases=len(submission_test_cases),
        )
    if submission.active_contest_id is not None:
...
def get_user_submissions(
    ...
    submissions = session.exec(
        select(Submission)
        .where(Submission.user_id == user.id)
        .order_by(Submission.created_at.desc())  # pyright: ignore[reportAttributeAccessIssue]
        .offset(PAGE["MEDIUM"] * (body.current_page - 1))
        .limit(PAGE["MEDIUM"])
    ).all()
    if submissions is None:
...
def get_submission_info(
    ...
    if submission is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No info found for submission"
        )
    ...
    if submission.status == SubmissionStatusId.WA:
        test_case = session.exec(
            select(TestCase)
            .where(TestCase.submission_id == submission_id)
            .where(TestCase.status == SubmissionStatusId.WA)
        ).first()
        if test_case is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No test cases found for submission"
            )
        return SubmissionStatusBase(
    ...
    if submission.status == SubmissionStatusId.CE:
        test_case = session.exec(
            select(TestCase)
            .where(TestCase.submission_id == submission_id)
            .where(TestCase.status == SubmissionStatusId.CE)
        ).first()
        if test_case is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No test cases found for submission"
    ...
    if submission.status == SubmissionStatusId.TLE:
        test_case = session.exec(
            select(TestCase)
            .where(TestCase.submission_id == submission_id)
            .where(TestCase.status == SubmissionStatusId.TLE)
        ).first()
        if test_case is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="No test cases found for submission"
    ...
    test_case = session.exec(
        select(TestCase)
        .where(TestCase.submission_id == submission_id)
        .where(TestCase.status == submission.status)
    ).first()
    if test_case:
        return SubmissionStatusBase(
            stderr=test_case.stderr,
